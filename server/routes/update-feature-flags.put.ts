...
export default eventHandler(async (event) => {
  ...
  const { featureFlags, userId } = await zodValidateBody(event, requestBodySchema.parse);
  ...
  if (userId && !(
    ...
    can(manager, "update-managed-users-feature-flags") && user.meta?.get("managerId") === manager.id
  )) {
    throw createError({ message: "Unauthorized to update target user feature flags!", status: 403 });
  }
