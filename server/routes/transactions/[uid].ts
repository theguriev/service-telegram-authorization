...
export default eventHandler(async (event) => {
	...
	const validated = await zodValidateData(query, querySchema.parse);
	...
	const validatedOwnerId = await zodValidateData(ownerId, userIdSchema.parse);
	...
	if (
		!(
			...
			(owner.meta?.get("managerId") === user.id &&
				can(user, "get-managed-users-transactions"))
		)
	) {
		throw createError({
