...
const calculateCurrentBalance = (
	...
	const sortedTransactions = sortBy(transactions, [
	...
	const { currentBalance, previousTransaction } = sortedTransactions.reduce<{
		...
		({ previousTransaction, currentBalance }, transaction) => {
			...
			const previousTransactionDate = getStartDate(
				new Date(previousTransaction.timestamp),
			);
			...
			const value =
				Math.max(0, currentBalance - daysDifference) + transaction.value;
			return {
				previousTransaction: transaction,
