...
export default async function getBalance<const T extends string>(
	addresses: T[],
	currencySymbol: string,
...
export default async function getBalance<const T extends string>(
	...
	if (isString(address)) {
		const balance = await $fetch<Record<string, number | undefined>>(
	...
	const limit = 100;
	const asyncBalances = Array.from(
		{ length: Math.ceil(address.length / limit) },
		async (_, offset) => {
			const chunk = address.slice(offset * limit, (offset + 1) * limit);
			const { results } = await $fetch<BalancesResponse<T>>(
				`/billing/ballance`,
				{
					method: "POST",
					body: {
			...
			return Object.values(results).reduce<Record<T, number>>(
				(acc, { address, ballanceBySymbol }) => ({
					...acc,
