...
const getStartDate = (date: Date) => {
	const zonedDate = new Date(
		toZonedTime(date, "Europe/Kyiv").getTime() - dateDifference.valueOf(),
	);
...
const queries: Record<string, QueryFunc> = {
	...
	getReportQuery: ({ report }) => {
		...
		if (
			weekends.some(
				(weekend) => toZonedTime(startDate, "Europe/Kyiv").getDay() === weekend,
			)
		) {
...
const getUsers = async (
	...
	validated: z.infer<typeof usersRequestSchema>,
	withOffsets: boolean = true,
) => {
