...
export default defineTask({
	meta: {
		...
		description:
			"Daily task to notify managers about users with expiring subscriptions",
	},
	async run() {
		...
		const balances = await getBalance(
			result.flatMap(({ users }) => users.map((user) => user.address)),
			currencySymbol,
		);
		for (const { managerId, users } of result) {
			...
			try {
				...
				console.error(
					`Error sending notification to manager ${managerId}:`,
					error,
				);
			}
		}
